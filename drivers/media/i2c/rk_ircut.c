...
static int ap1511a_parse_dt(struct ircut_dev *ircut, struct device_node *node)
{
    ...
    ircut->close_gpio = devm_gpiod_get(ircut->dev, "ircut-close", GPIOD_OUT_LOW);
	if (IS_ERR(ircut->close_gpio)) {
		dev_err(ircut->dev, "Failed to get ircut-close-gpios\n");
		return PTR_ERR(ircut->close_gpio);
	}
    ...
    ircut->tof_gpio = devm_gpiod_get_optional(ircut->dev, "tof", GPIOD_OUT_HIGH);
	if (IS_ERR(ircut->tof_gpio)) {
		dev_err(ircut->dev, "Failed to get tof-gpios\n");
	}
   
    if (ircut->tof_gpio) {
        gpiod_direction_output(ircut->tof_gpio, 1);
        gpiod_set_value(ircut->tof_gpio, 1); 
    }
